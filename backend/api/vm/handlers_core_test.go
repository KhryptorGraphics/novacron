...
func newAPIVMTestManager(t *testing.T) *corevm.VMManager {
	t.Helper()
...
func TestListVMs_DefaultsAndEnvelope(t *testing.T) {
	...
	if rec.Header().Get("X-Pagination") == "" {
		t.Fatalf("missing X-Pagination header")
	}
...
func TestListVMs_CustomParamsAndValidation(t *testing.T) {
	...
	for i, id := range ids {
		v, _ := m.GetVM(id)
		if i%2 == 0 {
			v.SetState(corevm.StateRunning)
		}
		if i%3 == 0 {
			v.SetName("foo-" + v.Name())
		}
	}
	r := testRouter(t, m)
	...
	if rec.Header().Get("X-Pagination") == "" {
		t.Fatalf("missing pagination header")
	}
}
...
func TestCreateStartStopAndDeleteAsOperator(t *testing.T) {
	...
	// Create
	rec := makeRequest(r, "POST", "/api/v1/vms", "operator", map[string]any{"name": "vm-a", "command": "/bin/true", "owner_id": "owner-a", "tenant_id": "tenant-a"})
...
func TestUpdateVM_PatchValidation(t *testing.T) {
	...
	defer m.Stop()
	ctx := context.Background()
	v, err := m.CreateVM(ctx, corevm.CreateVMRequest{Name: "p1", Spec: corevm.VMConfig{Name: "p1", Command: "/bin/true", OwnerID: "owner-a", TenantID: "tenant-a"}})
	if err != nil {
		t.Fatalf("create vm: %v", err)
	}
	r := testRouter(t, m)
