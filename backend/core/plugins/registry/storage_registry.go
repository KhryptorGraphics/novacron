...
import (
	...
	ceph "github.com/khryptorgraphics/novacron/backend/core/plugins/storage/ceph"
	"github.com/khryptorgraphics/novacron/backend/core/plugins/storage/netfs"
	objectstorage "github.com/khryptorgraphics/novacron/backend/core/plugins/storage/object"
...
var StorageRegistry = &StoragePluginRegistry{
	plugins:     make(map[string]map[string]interface{}),
	initialized: false,
}

// InitializeStorageRegistry initializes the storage plugin registry
func InitializeStorageRegistry() error {
	StorageRegistry.mu.Lock()
	defer StorageRegistry.mu.Unlock()

	if StorageRegistry.initialized {
		return nil
	}

	// Create plugin type map if it doesn't exist
	if _, exists := StorageRegistry.plugins["StorageDriver"]; !exists {
		StorageRegistry.plugins["StorageDriver"] = make(map[string]interface{})
	}

	// Register Ceph storage driver
	StorageRegistry.plugins["StorageDriver"][ceph.CephPluginInfo.Name] = ceph.CephPluginInfo

	// Register Network File System storage driver
	StorageRegistry.plugins["StorageDriver"][netfs.NetworkFSPluginInfo.Name] = netfs.NetworkFSPluginInfo

	// Register Object Storage driver
	StorageRegistry.plugins["StorageDriver"][objectstorage.ObjectStoragePluginInfo.Name] = objectstorage.ObjectStoragePluginInfo

	// Register factories with the core storage system
	registerStorageDriverFactories()

	StorageRegistry.initialized = true
	log.Println("Storage plugin registry initialized with", len(StorageRegistry.plugins["StorageDriver"]), "storage drivers")
	return nil
}
