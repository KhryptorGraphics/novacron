...
func TestLiveMigrationOrchestrator(t *testing.T) {
	...
	t.Run("ConcurrentMigrations", func(t *testing.T) {
		...
		// A completed migration drains the queue asynchronously; by this point
		// it may be 0 or 1 — asserting exactly 1 races the completion path.
	...
	t.Run("MigrationCancellation", func(t *testing.T) {
		...
		// CancelMigration is honest about its failure modes: the migration
		// already finished and left the active set ("migration not found"),
		// or it is still pending with no checkpoint ("cannot cancel migration
		// without checkpoint"). Both are documented behavior, not failures.
		if err != nil {
			assert.True(t,
				err.Error() == "cannot cancel migration without checkpoint" ||
					err.Error() == "migration not found",
				"unexpected cancel error: %v", err)
		}
	})
...
func TestRollbackManager(t *testing.T) {
	...
	t.Run("ExecuteRollback", func(t *testing.T) {
		...
		// Verify the completed rollback was recorded: the rollback lifecycle
		// ends in the "cleanup" step removing the active entry (by design, the
		// rollback no longer runs), and completion is durable in the
		// transaction log and reflected in the metrics collector.
		assert.NotEmpty(t, manager.transactionLog.transactions)
		var completed bool
		for _, tx := range manager.transactionLog.transactions {
			if tx.Operation == "rollback_completed" && tx.MigrationID == "migration-2" && tx.State == "success" {
				completed = true
			}
		}
		assert.True(t, completed, "expected a rollback_completed transaction for migration-2")
		metrics := manager.metricsCollector.GetMetrics()
		assert.Equal(t, 1, metrics["total_rollbacks"])
...
func TestBandwidthManager(t *testing.T) {
	t.Run("AllocateAndRelease", func(t *testing.T) {
		...
		// Existing allocations still consume bandwidth, so only the remaining
		// capacity (1000 - 500 held by migration-2) is available.
...
func getTestConfig(t *testing.T) MigrationConfig {
