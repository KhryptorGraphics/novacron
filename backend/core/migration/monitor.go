...
// GetDashboardData returns data for the migration dashboard. It refreshes
// from the live migration set first: the background updateLoop only ticks
// once per second, so a read right after StartMonitoring would otherwise
// return a stale snapshot missing the just-registered migrations.
...
// NewPrometheusExporter creates a new Prometheus exporter. Multiple
// orchestrators can be constructed in one process (tests, sidecar tools), and
// each exporter owns its own prometheus.Registry: re-registering the same
// metric names into the default registry panics on the second instance.
func NewPrometheusExporter() *PrometheusExporter {
	...
	return &PrometheusExporter{
		...
		migrationsActive: factory.NewGauge(prometheus.GaugeOpts{
		...
		migrationProgress: factory.NewGaugeVec(prometheus.GaugeOpts{
			Name: "novacron_migration_progress",
			Help: "Migration progress percentage",
		}, []string{"migration_id"}),
		transferRate: factory.NewGaugeVec(prometheus.GaugeOpts{
