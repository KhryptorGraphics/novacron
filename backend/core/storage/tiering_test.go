...
// tieringTestConfig returns the default tiering config with every tier's
// local driver pointed at an isolated temp directory so tests never touch the
// default storage root or share state with other runs.
func tieringTestConfig(t testing.TB, name string) TieringConfig {
	config := DefaultTieringConfig()
	basePath := filepath.Join(t.TempDir(), name)
	for i := range config.Tiers {
		// DefaultTieringConfig points cold/archive tiers at the (unregistered)
		// "s3" driver; every tier uses the local driver here, isolated per
		// tier under the temp dir.
...
func TestTieringManager_CreateAndStart(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_CreateAndStart")
...
func TestTieringManager_RecordAccess(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_RecordAccess")
...
func TestTieringManager_CalculateAccessFrequency(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_CalculateAccessFrequency")
...
func TestTieringManager_ShouldPromote(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_ShouldPromote")
...
func TestTieringManager_ShouldDemote(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_ShouldDemote")
	...
	// Test no demotion due to minimum storage duration. The hot tier has
	// MinStorageDuration 0 (no minimum), so exercise the guard with the warm
	// tier (30 days) instead.
	stats.LastAccessed = oldTime
	warmConfig := tieringManager.tiers[TierWarm]
	stats.TierChangeTime = time.Now().Add(-30 * time.Minute) // Recently moved to warm
	stats.CurrentTier = TierWarm
	targetTier = tieringManager.shouldDemote(stats, warmConfig)
...
func TestTieringManager_GetTierStats(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_GetTierStats")
...
func TestTieringManager_CalculateTierCosts(t *testing.T) {
	config := tieringTestConfig(t, "TestTieringManager_CalculateTierCosts")
...
func BenchmarkTieringManager_RecordAccess(b *testing.B) {
	config := tieringTestConfig(b, "BenchmarkTieringManager_RecordAccess")
...
func BenchmarkTieringManager_CalculateAccessFrequency(b *testing.B) {
	config := tieringTestConfig(b, "BenchmarkTieringManager_CalculateAccessFrequency")
...
func BenchmarkTieringManager_GetTierStats(b *testing.B) {
	config := tieringTestConfig(b, "BenchmarkTieringManager_GetTierStats")
